...
<plugin key="AMZC" name="AC Main duct control in multizone central duct air system" author="Erwanweb" version="0.0.3" externallink="https://github.com/Erwanweb/AMZC.git">
    ...
    <params>
        ...
        <param field="Username" label="CAC221 mode, fan, setpoint idx (csv list of idx)" width="200px" required="true" default=""/>
...
class BasePlugin:
    ...
    def buildCommandString(self):
        ...
        if (Devices[4].nValue == 1):
            ...
            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=10".format(self.WACmode))
        ...
        if self.turbo :
            ...
            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=40".format(WACfanspeed))
        else:
            #for idx in self.WACfanspeed:
            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=10".format(WACfanspeed))
        ...
        if (Devices[4].nValue == 1):
            ...
            DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint=17".format(WACsetpoint))
        elif (Devices[3].nValue == 1):
            # general heating request....
            #for idx in self.WACsetpoint:
            DomoticzAPI("type=command&param=setsetpoint&idx={}&setpoint=28".format(WACsetpoint))
        else :
            # off
            #for idx in self.WACmode:
            DomoticzAPI("type=command&param=switchlight&idx={}&switchcmd=Set Level&level=0".format(self.WACmode))
