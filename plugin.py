...
<plugin key="AMZC" name="AC Main duct control in multizone central duct air system" author="Erwanweb" version="0.0.2" externallink="https://github.com/Erwanweb/AMZC.git">
    ...
    <params>
        ...
        <param field="Mode5" label="Delay : On, order confirm. (all in minutes)" width="200px" required="true" default="1,5"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 2:
            ...
            if self.ondelay < 1:
                ...
                self.ondelay = 1
            self.orderconfirm = CheckParam("order confirmation Delay",params[1],5)
            if self.orderconfirm < 5:
                Domoticz.Error("Invalid forced order confirmation parameter. Using minimum of 5 minutes !")
                self.orderconfirm = 5
    ...
    def onHeartbeat(self):
        ...
        if Devices[1].sValue == "0":  # Control is off
            ...
            if self.Air:  # thermostat setting was just changed so we kill the heating
                self.Air = False
                self.Airgrequested = False
                Domoticz.Debug("Switching main duct Off !")
                if Devices[2].nValue == 1:
                   Devices[2].Update(nValue = 0,sValue = Devices[2].sValue)
                if Devices[3].nValue == 1:
                   Devices[3].Update(nValue = 0,sValue = Devices[2].sValue)
                if Devices[4].nValue == 1:
                   Devices[4].Update(nValue = 0,sValue = Devices[2].sValue)
        else:
            self.Airrequest()
                    ...
                    if not self.Heatingrequestregistred:
                        ...
                        if self.Airrequestchangedtime + timedelta(minutes=self.ondelay) < now:
                            ...
                            if Devices[2].nValue == 0: # no zone in heating, so we are in cooloning mode....
                                Devices[4].Update(nValue = 0,sValue = Devices[4].sValue) # general cooling request....
                            else :
                                Devices[3].Update(nValue = 0,sValue = Devices[3].sValue) # general cooling request....
